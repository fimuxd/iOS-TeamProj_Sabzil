...
//  Plot
//
//  Created by Bo-Young PARK on 31/7/2017.
...
struct ExibitionData {
    ...
    let exibition_ComentsFromUser:[Coment]  //후기
...
struct UserData {
    ...
    let user_Conments:[Coment]              //후기
}

struct StarPoint {
    let starPoint_ID:Int                    //기본적으로 부여되는 ID 값
    let starPoint_ExibitionID:Int           //별점을 준 전시ID
    let starPoint_Point:Int                 //별점
}

struct Coment {
    let coment_ID:Int                       //기본적으로 부여되는 ID 값
    let coment_Detail:String                //후기
}
